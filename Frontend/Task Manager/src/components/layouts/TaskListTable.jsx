          ...
          {tableData.map((task) => {
            return (
              <tr key={task._id} className="border-t border-gray-200">
                <td className="my-3 mx-4 text-gray-[13px] line-clamp-1 overflow-hidden">
                  {task.title}
                </td>
                <td className="p-4">
                  <span
                    className={`px-2 py-1 text-xs rounded inline-block ${getStatusBudgetColor(
                      task.status
                    )}`}
                  >
                    {task.status}
                ...
                <td className="py-3 px-4 text-gray-700 text-nowrap text-[13px] hidden md:table-cell">
                  <span
